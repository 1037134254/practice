...
func GetSubmitList(c *gin.Context) {
	size, err := strconv.Atoi(c.DefaultQuery("size", defaults.DefaultSize))
	page, err := strconv.Atoi(c.DefaultQuery("page", defaults.DefaultPage))
	if err != nil {
		return
	}
	page = (page - 1) * size
...
func Submit(c *gin.Context) {
	...
	for _, testCase := range pb.TestCases {
		...
		go func() {
			...
			// 运行超内存
			if em.Alloc/1024-(bm.Alloc/1024) > uint64(pb.MaxMen) {
				OOM <- 1
				return
			}
			lock.Lock()
	...
	if err = models.DB.Transaction(func(tx *gorm2.DB) error {
